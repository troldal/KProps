...
//                return PROPERTY_T { 0.0 };
        ...
        auto property(Property prop) const {
            PropertyVariant var = mapPropertyToVariant(prop);
            return std::visit([&](auto&& arg) {
                using PROPERTY_T = std::decay_t<decltype(arg)>;
                return property<PROPERTY_T, UNITS_T>().get(); }, var);
        }

        template<typename UNITS_T = MolarUnits>
        [[nodiscard]]
        auto property(const std::string& propertyString) const {
            return property<UNITS_T>(mapStringToProperty(propertyString));
        }
