...
//
#pragma once
#if not defined(__cpp_variadic_templates) or not defined(__cpp_rvalue_references) or not defined(__cpp_decltype) or        \
    not defined(__cpp_alias_templates) or not defined(__cpp_generic_lambdas) or not defined(__cpp_constexpr) or            \
    not defined(__cpp_return_type_deduction) or not defined(__cpp_fold_expressions) or not defined(__cpp_static_assert) or \
        ...
        namespace te
        {
            inline namespace v1
            {
                namespace detail
                {
                    ...
                    constexpr inline T exchange(T& obj, U&& new_value) noexcept(std::is_nothrow_move_constructible<T>::value &&
                ...
                {
                    template<class T, class T_ = std::decay_t<T>, std::enable_if_t<!std::is_same_v<T_, non_owning_storage>, bool> = true>
                ...
                {
                    template<class T, class T_ = std::decay_t<T>, std::enable_if_t<!std::is_same_v<T_, shared_storage>, bool> = true>
                ...
                {
                    template<class T, class T_ = std::decay_t<T>, std::enable_if_t<!std::is_same_v<T_, dynamic_storage>, bool> = true>
                ...
                struct local_storage
                {
                    ...
                    template<class T, class T_ = std::decay_t<T>, std::enable_if_t<!std::is_same_v<T_, local_storage>, bool> = true>
                ...
                struct sbo_storage
                {
                    ...
                    constexpr explicit sbo_storage(T&& t) noexcept(std::is_nothrow_constructible_v<T_, T&&>)
                        : ptr { new(&data) T_ { std::forward<T>(t) } },
                          del { [](mem_t& self_mem, void*) { reinterpret_cast<T_*>(&self_mem)->~T_(); } },
                    ...
                    constexpr explicit sbo_storage(T&& t) noexcept(std::is_nothrow_constructible_v<T_, T&&>)
                        : ptr { new T_ { std::forward<T>(t) } },
                          del { [](mem_t&, void* self_ptr) { delete reinterpret_cast<T_*>(self_ptr); } },
                ...
                {
                public:
                    template<class T, class T_ = std::decay_t<T>, std::enable_if_t<!std::is_same_v<T_, poly>, bool> = true>
                ...
                namespace detail
                {
                    ...
                    constexpr auto
                        call_impl(const poly_base& self, std::integral_constant<std::size_t, N>, type_list<R>, const TExpr, Ts&&... args)
                    {
                        void(typename mappings<I, N>::template set<type_list<TExpr, Ts...>> {});
                        return reinterpret_cast<R (*)(void*, Ts...)>(self.vptr[N - 1])(self.ptr(), std::forward<Ts>(args)...);
                    }
                    ...
                    constexpr auto requires_impl(type_list<TExpr, Ts...>) -> decltype(&TExpr::template operator()<T, Ts...>);
                ...
                {
                    static_assert(std::is_empty<TExpr> {});
                    return detail::call_impl<I>(reinterpret_cast<const detail::poly_base&>(interface),
...
#    if defined(__cpp_concepts)
                template<class I, class T>
                concept var = requires { detail::requires_impl<I, T>(std::make_index_sequence<detail::mappings_size<T, I>()> {}); };

                template<class I, class T>
                concept conceptify = requires { detail::requires_impl<I, T>(std::make_index_sequence<detail::mappings_size<T, I>()> {}); };
#    endif
